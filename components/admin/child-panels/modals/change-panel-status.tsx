...
interface ChangePanelStatusModalProps {
  isOpen: boolean;
  panelId: number;
  currentStatus: string;
  onClose: () => void;
  onConfirm: (panelId: number, status: string, reason: string) => void;
}

const ChangePanelStatusModal: React.FC<ChangePanelStatusModalProps> = ({
  isOpen,
  panelId,
  currentStatus,
  onClose,
  ...
  const handleConfirm = () => {
    if (statusReason.trim() && newStatus !== currentStatus) {
      onConfirm(panelId, newStatus, statusReason);
    }
  };
...
export default ChangePanelStatusModal;
