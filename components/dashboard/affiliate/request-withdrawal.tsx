...
export function RequestWithdrawalModal({
  ...
  const formatAmount = (amount: number): string => {
    if (currencySettings && availableCurrencies.length > 0) {
      return formatCurrencyAmount(amount, 'USD', availableCurrencies, currencySettings);
    }
        ...
        <div className="max-h-[90vh] overflow-y-auto p-6">
          ...
          <div className="space-y-4">
            <div className={`p-4 rounded-lg border ${hasError ? 'bg-red-50 border-red-200 dark:bg-red-900/20 dark:border-red-800' : 'bg-green-50 border-green-200 dark:bg-green-900/20 dark:border-green-800'}`}>
              ...
              {!hasError && (
                <div className="text-xs text-green-600 dark:text-green-400 mt-1">
                  Original balance: {availableBalanceDisplay || '$0.00'} • Minimum withdrawal: {formatAmount(minWithdrawalValue)}
                </div>
              )}
              {hasError && (
            ...
            <div>
              ...
              {hasError && (
                <div className="text-xs text-red-600 dark:text-red-400 mt-1 font-medium">
                  Available balance: {formatAmount(availableBalance)} • You're trying to withdraw: {formatAmount(withdrawalAmount)}
                </div>
              )}
            </div>
            <div>
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <div className="text-xs text-blue-700 dark:text-blue-300">
                    ...
                    • Minimum withdrawal amount is {formatAmount(minWithdrawalValue)}<br />
