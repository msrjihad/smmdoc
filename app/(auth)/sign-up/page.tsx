...
export default function SignUpPage() {
  ...
  useEffect(() => {
    let cancelled = false;
    fetch('/api/public/user-settings')
      .then((res) => res.json())
      .then((data) => {
        if (cancelled) return;
        const enabled = data?.userSettings?.signUpPageEnabled ?? true;
        if (!enabled) window.location.href = '/sign-in?message=registration-disabled';
      })
      .catch(() => {});
    return () => {
      cancelled = true;
  ...
  const checkEmailAvailability = useCallback(async (email: string) => {
    ...
    try {
      ...
      const result = await response.json();
      if (result.available) {
        setEmailStatus({ checking: false, available: true, message: 'Email is available' });
      } else {
        setEmailStatus({
          checking: false,
          available: false,
