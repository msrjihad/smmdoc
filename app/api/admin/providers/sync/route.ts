          ...
          try {
            ...
            if (existingService) {
              ...
              if (syncType === 'all') {
                ...
                const minBigInt = BigInt(minValue || 100);
                if (minBigInt !== existingService.min_order) {
                  updates.min_order = minBigInt;
                  hasChanges = true;
                }

                const maxValue = typeof providerService.max === 'string' ? parseInt(providerService.max) : providerService.max;
                const maxBigInt = BigInt(maxValue || 10000);
                if (maxBigInt !== existingService.max_order) {
                  updates.max_order = maxBigInt;
                  hasChanges = true;
                }
