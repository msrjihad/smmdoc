...
export async function PUT(request: Request) {
  try {
    ...
    if (categoryId !== undefined && categoryId !== null && categoryId !== '') {
      ...
      if (categoryIdInt === undefined || categoryIdInt < 1) {
        return NextResponse.json({
      ...
      if (!categoryExists) {
        return NextResponse.json({
          error: 'The selected category does not exist. Please select a valid category.',
          data: null,
          success: false,
        }, { status: 400 });
      }
    ...
    if (min_order !== undefined && min_order !== null && min_order !== '') {
      updateData.min_order = BigInt(Math.floor(toNumber(min_order, 0)));
    }
    if (max_order !== undefined && max_order !== null && max_order !== '') {
      updateData.max_order = BigInt(Math.floor(toNumber(max_order, 0)));
    }
