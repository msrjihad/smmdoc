...
export async function GET(request: Request) {
  try {
    ...
    const MAX_SERVICES_LIMIT = 2000;
    const limit = Math.min(isNaN(rawLimit) ? 10 : rawLimit, MAX_SERVICES_LIMIT);
    ...
    if (limit >= 500) {
      ...
      console.log('Fetching services (limit >= 500, capped at', MAX_SERVICES_LIMIT, ') with whereClause:', JSON.stringify(whereClause, null, 2));
    ...
    const servicesTake = Math.min(limit * 50, MAX_SERVICES_LIMIT);
    const services = await db.services.findMany({
      where: whereClause,
      orderBy: {
        createdAt: 'desc',
...
export async function POST(request: Request) {
  try {
    ...
    const createData: any = {
      ...
      min_order: BigInt(Math.floor(toNumber(min_order, 0))),
      max_order: BigInt(Math.floor(toNumber(max_order, 0))),
    ...
    const safeStringify = (obj: unknown) => JSON.stringify(obj, (_, v) => (typeof v === 'bigint' ? v.toString() : v), 2);
    console.log('Creating service with data:', safeStringify(createData));
