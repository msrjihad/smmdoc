...
export async function POST(request: Request) {
  try {
    ...
    const uniqueServiceIds = [...new Set(orders.map((o: any) => o.serviceId).filter(Boolean))];
    const servicesList = uniqueServiceIds.length > 0
    ...
    for (let i = 0; i < orders.length; i++) {
      ...
      try {
        ...
        const servicesDiscount = Number(user.servicesDiscount) || 0;
        const effectiveRate = servicesDiscount <= 0 ? service.rate : service.rate * (1 - servicesDiscount / 100);
        const usdPrice = (effectiveRate * quantity) / 1000;
    ...
    const result = await db.$transaction(async (prisma) => {
      ...
      for (const orderData of validatedOrders) {
        const { orderIndex, service, isMassOrder, batchId, ...createData } = orderData;
    ...
    console.log(
      `User ${session.user.email} created ${result.length} Mass Orders`,
      {
        userId: session.user.id,
    ...
    return NextResponse.json(
      {
        ...
        data: result.map(serializeOrder),
        summary: {
    ...
    return NextResponse.json(
      {
        success: false,
        message: errMsg || 'Error creating Mass Orders',
        error: errMsg,
      },
      { status: 500 }
    );
  ...
  try {
    ...
    if (type === 'recent') {
      ...
      return NextResponse.json(
        {
          success: true,
          data: recentOrders.map(serializeOrder),
        },
        { status: 200 }
      );
    }
