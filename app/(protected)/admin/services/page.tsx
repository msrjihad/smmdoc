...
function AdminServicesPage() {
  ...
  const { data: allServicesData } = useSWR(
    `/api/admin/services?page=1&limit=2000&search=&filter=all_with_trash`,
    fetcher,
    {
      revalidateOnFocus: false,
      refreshInterval: 0,
  ...
  const toggleCategoryAndServices = async (
    ...
    try {
      ...
      if (newToggleState) {
        setCollapsedCategories((prev) => prev.filter((c) => c !== categoryName));
      }
  ...
  useEffect(() => {
    if (data?.data && Object.keys(groupedServices).length > 0) {
      ...
      Object.keys(groupedServices).forEach((categoryName) => {
        const categoryIdMatch = categoryName.match(/\(ID:\s*(\d+)\)/);
        const categoryData = categoryIdMatch
          ? categoriesData?.data?.find((cat: any) => cat.id === parseInt(categoryIdMatch[1]))
          : categoriesData?.data?.find((cat: any) => cat.category_name === getActualCategoryName(categoryName));
        if (categoryData?.hideCategory === 'yes') categoriesToCollapse.push(categoryName);
      });
      if (categoriesToCollapse.length > 0) {
        setCollapsedCategories((prev) => {
          const next = new Set(prev);
          categoriesToCollapse.forEach((c) => next.add(c));
      ...
      const allCollapsed =
        ...
        allCategoryNames.every((cat) => categoriesToCollapse.includes(cat));
