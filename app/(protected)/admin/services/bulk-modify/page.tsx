...
import { SelectCategoryModal } from '@/components/admin/services/bulk-modify/modals/select-category';

const ServicesTable = dynamic(
  () => import('@/components/admin/services/bulk-modify/services-table').then(
    (m) => m.ServicesTable
  ),
  { ssr: false }
);
...
const BulkModifyPage = () => {
  ...
  const handleSaveChanges = async () => {
    try {
      ...
      const useBulkUpdate = allServicesSameChanges && firstServiceChanges && Object.keys(updateData).length > 0;
      ...
      if (useBulkUpdate) {
        ...
        const updatePromises = Object.entries(editedServices).map(async ([serviceId, changes]) => {
          ...
          const categoryId = changes.categoryId ?? originalService.categoryId ?? (originalService as any).category?.id;

          const updatedData = {
            name: changes.name ?? originalService.name,
            description: changes.description ?? originalService.description,
            rate: (changes.rate ?? originalService.rate)?.toString?.() ?? String(originalService.rate),
            min_order: (changes.min_order ?? originalService.min_order)?.toString?.() ?? String(originalService.min_order),
            max_order: (changes.max_order ?? originalService.max_order)?.toString?.() ?? String(originalService.max_order),
            categoryId,
            serviceType: changes.service_type ?? changes.serviceType ?? originalService.service_type ?? (originalService as any).serviceType ?? 'other',
            mode: 'manual',
    ...
    <div className="page-container">
      ...
      <div className="page-content">
          ...
          <div className="px-6 py-6">
            ...
            {(localServicesLoading || isInitialLoading || paginationLoading) ? (
              ...
              <ServicesTable
                ...
                localServicesLoading={localServicesLoading}
        ...
        <SelectCategoryModal
          ...
          onCategoryChange={setTempSelectedCategory}
